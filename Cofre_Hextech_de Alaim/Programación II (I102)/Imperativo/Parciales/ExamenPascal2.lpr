...
begin
     ...
     while (aux <> nil ) do begin
          if (cant < aux^.dato.cantIntervecion) then begin
             cant:= aux^.dato.cantIntervecion;
             min:= c.direccion;
          end;
          aux:= aux^.sig
     end;
...
begin
     ...
     while( aux <> nil) do begin
            if (aux^.dato.peso >= 10) and (aux^.dato.fechaIngreso.anho = 2023) and ((aux^.dato.fechaIngreso.mes >= 1) and (aux^.dato.fechaIngreso.anho <= 3)) then begin
               writeln('Identificador: ', aux^.dato.identificador, ' tipo de mascota: ',aux^.dato.tipo);
            end;
            aux:= aux^.sig;
     end;
end;
