...
type
   ...
   Procedure busquedaAcotadaArbol(a: arbol; inf, sup: strCuit);
      begin
      if (a <> nil) then
         if (a^.cuit >= inf) then
   ...
   procedure cumple(a: arbol);
      ...
      begin
         ...
         prom:= montoTot / cont;
         Writeln(' - Cuit: ',a^.cuit, ' - Numero de Tarjeta: ', a^.pasajero.numTarj, ' - Promedio Gastado: ', prom:2:2);
      end;

   procedure encontrarPersonas(a: arbol; caso1, caso2: integer);
      begin
         if (a <> nil) then begin
            encontrarPersonas(a^.HI, caso1, caso2);
            if ((a^.pasajero.numTarj mod 10) = caso1) or ((a^.pasajero.numTarj mod 10) = caso2) then
               cumple(a);
            encontrarPersonas(a^.HD, caso1, caso2);
         end;
      end;

{
   // ---------------------- DEBUG ----------------------
   // LOS SIGUIENTES MODULOS NO FORMAN PARTE DE LA CONSIGNA, DECIDI INCLUIRLOS PARA QUE PUEDAN VER COMO CARGUE LA ESTRUCTURA
...
var
   ...
   incisoD_caso1, incisoD_caso2: integer;
BEGIN
   a:= nil;
...
*        // NO FORMA PARTE DE LA CONSIGNA
         ...
         writeln('Personas cuyo cuit se encuentra entre ', infBusqueda, ' y ', supBusqueda);
         busquedaAcotadaArbol(a, infBusqueda, supBusqueda);

         // INCISO D
         // en caso que se quiera leer desde teclado solo sustituir las declaraciones de incisoD con lectura de teclado de las mismas variables
         incisoD_caso1:= 4; incisoD_caso2:=5;
         writeln('Personas que tenien como ultimo digito de su numero de tarjeta SUBE el ', incisoD_caso1, ' o el ', incisoD_caso2,':');
         encontrarPersonas(a, incisoD_caso1, incisoD_caso2);
