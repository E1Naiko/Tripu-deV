...
begin
 if (a = nil) then begin
               new(a);
               a^.dato:= agregarAdelante(a^.dato, elemento);
               a^.HD:= nil;
               a^.HI:= nil;
   end else
     if (a^.dato^.dato.codigo > elemento.codigo) then
                 insertarEnRama(a^.HI, elemento)
     else if (a^.dato^.dato.codigo < elemento.codigo) then
			insertarEnRama(a^.HD, elemento)
