...
begin
 ...
 {--------------- procedimiento busca la cant maxima mensajes por usuario}



  procedure postOrden ( a : arbol ; var max :integer ; var nomb :string);
  begin
       if (a<>nil) then begin
        if ( a^.cant  > max )then  begin
         ...
         postOrden(a^.HI, max, nomb);
         postOrden(a^.HD, max, nomb);
