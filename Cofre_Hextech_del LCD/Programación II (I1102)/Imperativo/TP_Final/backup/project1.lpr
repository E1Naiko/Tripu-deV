...
procedure agregarnodo( var a:arboljugador; f:str10; j:jugador);
begin
     if (a = nil) then begin
        new(a);
        a^.dni:= j.dni;
        ...
        a^.partidos := nil;
        a^.totpuntaje := 0;
	agregarAdelante(a^.partidos,j.puntaje,f);
	a^.HD:= nil;
	a^.HI:= nil;
	end else
	    if (a^.dni > j.dni) then
	       agregarnodo(a^.HI,f,j)
	       else if (a^.dni < j.dni) then
	            agregarnodo(a^.HD,f,j)
		    else begin
		    agregarAdelante(a^.partidos,j.puntaje,f);
...
begin
        while (l <> nil) do begin
              ...
              while (jugador <> nil) do begin
                  agregarnodo(a,l^.dato.fecha,jugador^.dato);
                  jugador:= jugador^.sig;
              end;
              l:= l^.sig;
        end;
