...
{procedure imprimirdni (a:arbol; inf,sup:integer);
var
   aux:listacliente;
   begin
     if (a <> nil) then begin
        aux:= a^.servicio;
          if (a^.cod >= inf) and (a^.cod <= sup) then begin
               writeln('Codigo de area: ',a^.cod);
               writeln('Dnis de Clientes:');
               while (aux <> nil) do begin
                   write(aux^.dni,' | ');
                   aux:= aux^.sig;
                   end;
               end;
               imprimirdni(a^.HI,inf,sup);
               imprimirdni(a^.HD,inf,sup);
          end;
     end; }
